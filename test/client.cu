...
template<typename T>
void time_test(bool row_major, int m, int n,int rep) {
  ...
  float gbs = (float)(2 * m * n * sizeof(T)) / (time * 1000000)*rep;
...
int main(int argc, char**argv) {
  ...
  rep=atol(commandline_option(argc,argv,"-p","10",false,"-p <reps>"));
  ...
  if(d==1)
    time_test<double>(row_major, m, n,rep);
  else if(d==0)
    time_test<float>(row_major, m, n,rep);
}
