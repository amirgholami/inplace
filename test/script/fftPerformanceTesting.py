...
tableHeader = 'lengthx,lengthy,device,precision,label,gbs'
class TestCombination:
    ...
    def __str__(self):
        return self.x + 'x' + self.y + 'x' + ', ' + self.device + ', ' + ', ' + self.precision + ' -- ' + self.label
...
class GraphPoint:
    def __init__(self,
        ...
        self.precision = precision
        self.gbs = gbs
        self.problemsize = str(int(self.x) * int(self.y))

    def __str__(self):
        # ALL members must be represented here (x, y, z, batch, device, label, etc)
        return self.x + 'x' + self.y + 'x' + ':' + self.precision + ' precision, ' + self.device + ', -- ' + self.label + '; ' + self.gbs

class TableRow:
    # parameters = class TestCombination instantiation
    def __init__(self, parameters, gbs):
        self.parameters = parameters
        self.gbs = gbs

    def __str__(self):
        return self.parameters.__str__() + '; ' + self.gbs
...
def gemm_table_header():
    return 'm,n,k,lda,ldb,ldc,alpha,beta,order,transa,transb,function,device,library,label,gbs'
...
class GemmGraphPoint:
    ...
    def __str__(self):
        # ALL members must be represented here (x, y, z, batch, device, label, etc)
        return self.sizem + 'x' + self.sizen + 'x' + self.sizek + ':' + self.device + ', ' + self.function + ', ' + self.library + ', order(' + self.order + '), transa(' + self.transa + '), transb(' + self.transb + ') -- ' + self.label + '; ' + self.gbs + ' gbs'
