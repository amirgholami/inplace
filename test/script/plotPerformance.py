...
def plotFromDataFile():
  ...
  for thisFile in args.datafile:
    ...
    for row in resultsContents:
      row = row.split(',')
      row = TableRow(TestCombination(row[0],row[1],row[2],row[3],row[4]), row[5])
      data.append(GraphPoint(row.parameters.x, row.parameters.y, row.parameters.precision, row.parameters.device, row.parameters.label, row.gbs))
...
parser.add_argument('-y', '--y_axis',
  dest='graphyaxis', default='gbs', choices=yaxisvalues,
